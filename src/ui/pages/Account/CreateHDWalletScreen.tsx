      ...
      {RESTORE_WALLETS.map((item, index) => {
        return (
          <Button
            ...
            <Text color="black" text={item.name} />
          </Button>
        );
      })}
    </Column>
            ...
            return (
              <Row key={index}>
                ...
                <Card preset="style2" style={{ width: 200 }}>
                  <Text color="black" text={v} selectText />
                </Card>
              </Row>
            );
          })}
...
function Step1_Import({
  ...
  const [wordn, setWordn] = useState(12)
  ...
  return (
    <Column gap="lg">
      <Text text="Secret Recovery Phrase" preset="title-bold" textCenter />
      <Text color="white" text={`Import an existing wallet with your ${wordn} word secret recovery phrase`} preset="sub" textCenter />
      <Row justifyCenter>
        <Grid columns={2}>
          <Text text="12 words" onClick={() => setWordn(12)} style={{color:wordn == 12 ? 'rgb(0, 112, 186)' : 'black'}} />
          <Text text="24 words" onClick={() => setWordn(24)} style={{color:wordn == 24 ? 'rgb(0, 112, 186)' : 'black'}} />
        </Grid>
      </Row>
      <Row justifyCenter>
        <Grid columns={2}>
          {keys.map((_, index) => {
            return (
              <Row key={index}>
                <Card gap="zero">
                  <Text text={`${index + 1}. `} style={{ width: 25 }} textEnd color="black" />
                  <Input
